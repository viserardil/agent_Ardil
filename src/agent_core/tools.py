...
def resolve_tool_names(names=None):
    """Triyajın seçtiği araç adlarını kullanılabilir bir demete çevirir.

    Registry'de olmayan adlar sessizce elenir (triyaj uydurma bir ad döndürürse
    ajan kurulurken patlamasın). Sonuç boşsa — triyaj henüz devrede değil ya da
    bilinçli olarak kısıtlamamış — registry'nin tamamı döner.

    Demet döner çünkü ajan nesneleri araç kümesine göre lru_cache'lenir; anahtarın
    hashlenebilir olması gerekir.
    """
    selected = [name for name in (names or []) if name in TOOLS]
    return tuple(selected) if selected else tuple(TOOLS)


class _AgentToolInput(BaseModel):
    """Her araç çağrısının ortak girdi şeması.

    ``reason`` modelin o aracı NEDEN seçtiğini yazmaya MECBUR kaldığı alandır.
    Serbest metin ricasıyla (prompt) model tool-calling turlarında çoğunlukla hiç
    metin üretmiyordu — düşünce log'a düşmüyordu. Alanı şemada ZORUNLU yapınca
    gerekçe garanti geliyor. Araç fonksiyonu bu alanı KULLANMAZ; yalnızca koşu
    kaydına (tracing) geçer.

    ``reason`` bilerek ÖNCE tanımlı: model gerekçeyi girdiden önce yazsın, yani
    "düşünüp" sonra girdiyi belirlesin.
    """

    reason: str = Field(
        description="Bu aracı neden seçtiğin ve ne öğrenmeyi beklediğin, tek cümle."
    )
    tool_input: str = Field(
        description="Aracın işleyeceği serbest metin girdisi (ör. sembol, sorgu, dönem)."
    )
...
def get_tools(names=None):
    ...
    Her araca zorunlu bir ``reason`` alanı eklenir (bkz. _AgentToolInput); araç
    fonksiyonu onu yok sayar, tracing okur.
    """
    ...
    for name in selected:
        ...
        def _make(fn):
            # tool_input: modelin dolduracağı serbest metin. reason: yalnızca
            # gözlem/kayıt için; araç mantığına girmez.
            def _call(tool_input: str, reason: str = "") -> str:
